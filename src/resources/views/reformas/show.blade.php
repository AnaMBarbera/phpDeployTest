...
@section('content')
    ...
    <div class="container">
        <div class="post-main-image" style="position: relative; max-width: 100%; overflow: hidden;">
            @if($post->main_image)
                <img src="{{ asset('storage/'.$post->main_image) }}" alt="Imagen principal">
        ...
        @foreach($post->content as $para)
            <div class="post-paragraph">
                @if(!empty($para['image_left']))
                    <img src="{{ asset('storage/'.$para['image_left']) }}" alt="" class="image-left">
                @endif

                <p class="paragraph-text">{!! nl2br(e($para['text'])) !!}</p>

                @if(!empty($para['image_right']))
                    <img src="{{ asset('storage/'.$para['image_right']) }}" alt="" class="image-right">
                @endif
            </div>
        @endforeach
    </div>
